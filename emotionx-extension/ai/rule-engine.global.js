...
(function attachRuleEngine(global) {
  function modelBehavior(features, baseline) {
  ...
  function modelWebcam(features) {
    if (!features) return null;

    const { emotion = 'neutral', confidence = 0.55, blinkRate = 0, headMovement = 0, browTension = 0 } =
      features;

    const stabilityPenalty =
      0.1 * Math.max(0, headMovement - 0.5) + 0.1 * Math.max(0, blinkRate - 0.6);
  ...
  global.EmotionXRuleEngine = Object.assign(global.EmotionXRuleEngine || {}, {
    modelBehavior,
    modelWebcam
  });

  // Back-compat globals expected by `ai/emotion-engine.global.js`
  global.modelBehavior = global.modelBehavior || modelBehavior;
  global.modelWebcam = global.modelWebcam || modelWebcam;
})(globalThis);
